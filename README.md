...
# Building for ESP32 targets

Run devcontainer in VSCode, then source depedencies `. $IDF_PATH/export.sh`
`. scripts/activate.sh`

Easiest way to build example
` scripts/examples/esp_example.sh energy-management-app sdkconfig.defaults.esp32s3 esp32s3`

build directory will be placed inside `examples/energy-management-app/esp32`

simplest way to flash target is to use idf.py command generated by build tool
and outputed into build logs

example command will be looks like
` cd examples/energy-management-app/esp32 && python -m esptool --chip esp32s3 -b 460800 --before default_reset --after hard_reset write_flash --flash_mode dio --flash_size 4MB --flash_freq 80m 0x0 build/bootloader/bootloader.bin 0x8000 build/partition_table/partition-table.bin 0x1d000 build/ota_data_initial.bin 0x20000 build/chip-energy-management-app.bin`
